...
public class UserManager {
    ...
    public List<Course> selectLearnerClassesSQL(String learnerEmail) {
        String classCodeReturned;
        String creatorEmailReturned;
        String professorEmailReturned;
        String classNameReturned;
        ...
        try {
            PreparedStatement preparedStatement = conn.prepareStatement(
                    "SELECT class.classCode, creatorEmail, professorEmail, className, learningObj, learningOutcome, beginDate, endDate FROM class JOIN classListLookup ON classListLookup.classCode=class.classCode WHERE userEmail = ?");
            preparedStatement.setString(1, learnerEmail);
            rs = preparedStatement.executeQuery();
            while (rs.next()) {
                classCodeReturned = rs.getString(1);
                creatorEmailReturned = rs.getString(2);
